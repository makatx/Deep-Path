...
def MIA_InceptionResNetV2(input_image, num_out=2, activation='sigmoid'):
    base_model = InceptionResNetV2(weights=None, include_top=False, input_tensor=input_image, pooling=None)
    features = base_model.output
    x = layers.GlobalAveragePooling2D(name='avg_pool_frac')(features)
    x = layers.Dropout(0.1)(x)

    probabilities = layers.Dense(num_out, activation=activation, name='dense_duo_frac')(x)
